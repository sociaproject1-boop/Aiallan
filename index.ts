...
function shouldUseWebSearch(question: string) {
  ...
  return /\b(latest|today|current|now|recent|news|update|who is|what is|where is|when did|how many|price|weather|election|president|company|scientist|actor|athlete|technology|science|history|space|planet|ai|artificial intelligence|research|discovery|202\d|203\d)\b/i.test(q) || q.trim().length > 3;
}
